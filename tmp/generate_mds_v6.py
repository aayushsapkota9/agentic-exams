...
def get_sql_setup(text):
    ...
    for key, sql in SCHEMAS.items():
        if key in text_l:
            return sql
    # Detect generic SQL questions
    if "sql" in text_l or "relational algebra" in text_l:
        if any(x in text_l for x in ["join", "select", "where", "from", "delete", "update"]):
...
# --- STANDARD CHAPTER GENERATION (REVERT) ---

def generate_standard_chapter_md(ch_num, qns):
    title = chapter_names[ch_num]
    md = f"# Chapter {ch_num}: {title}\n\n"
    labels_list = sorted(list(set(q["label"] for q in qns)), key=label_key)
    
    if labels_list:
        md += "### Quick Navigation\n"
        nav_links = [f"[{label}](#{slugify(label)})" for label in labels_list]
        md += " | ".join(nav_links) + "\n\n---\n\n"
...
def generate_sqllab_md(all_qns):
    md = "# DBMS SQL Practice Lab\n\n"
    md += "This file contains a consolidated list of all practical SQL/Relational Algebra questions with copy-pastable database setup code.\n\n"
    
    # Identify SQL questions
    sql_qns = []
    for q in all_qns:
        setup = get_sql_setup(q["raw"])
        if setup:
            sql_qns.append((q, setup))
    
    # Sort by Chapter then Year
    sql_qns.sort(key=lambda x: (x[0]["chapter"], str(x[0]["year"])), reverse=False)
    
    current_chapter = None
    for q, setup in sql_qns:
        if q["chapter"] != current_chapter:
            current_chapter = q["chapter"]
            md += f"### Chapter {current_chapter}: {chapter_names[current_chapter]}\n\n"
        
        md += f"#### {q['year']} | Label: {q['label']}\n\n"
        md += "```markdown\n"
        md += f"{q['raw'].strip()}\n"
        md += "```\n\n"
        md += "```sql\n"
...
for i in range(1, 11):
    content = generate_standard_chapter_md(i, chapters[i])
    filepath = f"dbms/chap{i}/chap{i}-qns.md"
    with open(filepath, "w") as f:
        f.write(content)

# Final Consolidated SQL Lab
sqllab_content = generate_sqllab_md(questions)
with open("dbms/sqllab.md", "w") as f:
    f.write(sqllab_content)

print("Reverted unit chapters to clean state and generated consolidated 'dbms/sqllab.md'.")
