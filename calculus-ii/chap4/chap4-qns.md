...
. [7]
b) If $\vec{F} = 4xy \vec{i} - 8y \vec{j} + 3z \vec{k}$, find the line integral of $\vec{F}$ along the curve $y=3x, z=2x$ from $(0,0,0)$ to $(1,3,2)$. [8]
```
...
b) Show that a necessary and sufficient condition for a vector function of a scalar variable to have a constant magnitude is 
...
. [5]
b) Show that $\vec{V} = 2xyz^3 \vec{i} + x^2z^3 \vec{j} + 3x^2yz^2 \vec{k}$ is irrotational. [5]
```
